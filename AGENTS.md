...
| External APIs (maps, places, weather, booking, LLM providers) | `backend/trip_planner/inbound/` — the one integration that exists: AWS SES/SNS/S3 inbound mail. `ses.py` is the only module that knows AWS event shapes; `transport.py` is the normalised `ReceivedMessage` contract everything downstream consumes | Never commit API keys. Credentials come from environment variables — or, as the inbound adapter does, from the platform's own credential chain with a least-privilege role, which is better because no secret reaches `Settings` at all. Document each new variable in the README and in `.ai/qa/test-env.env.example`. Every external call needs a timeout, a byte cap and a defined failure mode: the S3 fetch has all three, and a failure leaves the message stored and retryable rather than lost. The integration is **optional** — unset configuration means the feature is off, so an existing deployment starts unchanged (§5). *Model providers are not wired yet: the reservation-inbox spec gates them behind a human check that no implementation engine can run.* |
