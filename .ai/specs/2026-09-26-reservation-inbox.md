...
backend/
  trip_planner/
    ...
    db/models.py                EXTENDED  InboundMessage, InboundActionItem, InboundCursor, InboundModelEgress;
...
**Three Alembic revisions**, each with a working `downgrade`. The first (Phase 1) creates `inbound_message`, `inbound_cursor`, adds `attachment.inbound_message_id` and **replaces** the two-parent `CHECK` with the three-parent one. The second (Phase 2) creates `inbound_model_egress`. The third (Phase 3) creates `inbound_action_item`.
