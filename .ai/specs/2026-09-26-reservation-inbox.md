...
| `external_uid` | TEXT NOT NULL | the mailbox's identity for the message: the RFC-5322 `Message-ID` when the message has one, else `uid:<n>`. |
| `uid_validity` | BIGINT NOT NULL DEFAULT 0 | part of the key, and **not decoration**: IMAP reassigns UIDs from 1 when a server changes `UIDVALIDITY`, so a key of `(owner_id, external_uid)` alone would let a *new* message collide with a stored UID and be swallowed by the very "insert is a no-op" that was supposed to be the safety net — mail loss wearing idempotency's clothes. `0` for a `Message-ID`-derived identity, which is already generation-independent |

`UNIQUE (owner_id, uid_validity, external_uid)` is **the whole of the idempotency story**: a re-poll, a crash between fetch and commit, or a restart cannot create a second copy, and a `UIDVALIDITY` change cannot hide a new message behind an old one.
...
| `text_sent_at` | TIMESTAMPTZ NULL | set the first time this message's **text** left the deployment. Document egress is stamped per document on `attachment.sent_externally_at`, because one nullable timestamp on the message cannot answer *which* documents have left — and "which" is exactly the distinction A2 is gated on. **The audit trail Q11's answer owes the owner**, so "what has been sent" is a query rather than a belief |
...
| `sent_externally_at` | TIMESTAMPTZ NULL | when **this document** left the deployment, if it ever did (A2). Per-attachment rather than per-message, so the inbox can say *which* files have been read off-box |
...
| `POST` | `/inbox/messages/{id}/place` | hand placement for the unrouted queue: `{trip_id, trip_day_id}` **re-points the message's documents directly** onto that day and marks the message routed. No action item is created and none is needed: this is the owner acting with his own hands, which is not the inbound write D21 governs. (With `interpret: true`, Phase 2 additionally re-runs routing against the named trip, and *that* produces an action item.) |
...
1. **A sender allow-list, with a way back.** A message is processed only when its `From` normalises to an address on the owner's allow-list (his own account address, plus any he adds). Everything else is **quarantined**: headers only — from, subject, date — with **no body stored and no attachment stored**, and purged after 30 days.
   ...
   **Quarantine is recoverable, and that is not a softening of the control.** The policy has one predictable false positive and it is not an edge case: a *mailbox rule* that auto-forwards airline confirmations preserves the airline's `From` and breaks SPF/DKIM alignment, so the exact setup that best serves P3 ("minimize manual work") is the one the allow-list rejects. A quarantine the owner cannot inspect would turn that into silent loss he discovers on a platform at 06:00. So the quarantine list shows **sender and subject** — his own mailbox's headers, shown to him alone — behind an explicit *Show* action, with a *Trust this sender* that adds the address and **re-fetches the message from the mailbox by its UID**, where it is still sitting. Nothing about a stranger's *content* is ever stored or displayed, which is the part that makes it a barrier rather than a delivery mechanism.
...
| Case | Behaviour |
|---|---|
| The same message fetched twice (re-poll, crash between fetch and commit, restart) | `UNIQUE (owner_id, uid_validity, external_uid)` — the second insert is a no-op. **The correctness boundary, not the cursor** |
| The IMAP server changes `UIDVALIDITY` | the cursor resets and the mailbox is re-read. Recognition is by `(uid_validity, external_uid)`, so a re-read duplicates nothing — **and a new message whose reassigned UID equals a stored one is still stored**, which a key without `uid_validity` would have swallowed as a false duplicate |
...
| A sender the owner does recognise is quarantined (an auto-forward keeping the airline's `From`) | visible in the quarantine list as sender and subject behind a *Show* action; *Trust this sender* adds the address and re-fetches the message from the mailbox. **A false positive is recoverable, not a number** |
...
| A4 | What does the app check about a sender? (the brief's **A09** ask) | **An allow-list of the owner's own addresses, *and* the mailbox provider's `Authentication-Results` showing a DMARC pass — checked on the headers before the body is downloaded. Anything else is quarantined: headers only, no body, no attachments, purged after 30 days — and the owner can see the sender and subject and release one with *Trust this sender*, which re-fetches it** | The sender is the only trust signal a forwarded message carries, and `From` alone is spoofable. Quarantining without storing content is what makes a stranger cost a few hundred bytes and no model call, and keeps the quarantine from becoming a delivery mechanism. **Why it is releasable:** the policy's predictable false positive is a mailbox *rule* that auto-forwards while keeping the airline's `From` — the setup that best serves P3 is the one the allow-list rejects — and an unrecoverable false positive there is silent loss discovered at an airport. **Stated limit:** this trusts the mailbox provider's own SPF/DKIM/DMARC evaluation; we do not re-verify signatures, and a provider writing no such header degrades the policy to allow-list only, logged at start-up |
...
- **Phase 4 — Polish, and the slippable phase.** The explicit "read this document" egress action, the quarantine *surface* (the list, *Show*, *Trust this sender*), the duplicate hint, and the manual re-check. **The quarantine's 30-day purge is not here — it is a growth-bounding control and ships in Phase 1**, with the policy it belongs to.
...
5. Message ingestion and **a trip-less `store_inbound_attachment`**: store the message, sniff each part through the **shipped** `domain/uploads.py`, write the metadata and blob rows parented to the message, drop and count what fails, quarantine a stranger with headers only. The shipped `store_attachment` is not reused — it requires a `Trip` an unrouted message does not have. Verify: tests that an accepted message stores its text and its PDF; that a stranger's message stores **no body and no attachment**; that an unsupported part is dropped while the message is kept; that the same `(uid_validity, external_uid)` twice inserts once **and that a reused UID under a new `uid_validity` inserts a second row**; and that a message whose every part fails still produces a readable message.
...
9. `api/inbox.py` — `GET /inbox/summary`, `GET /inbox/messages`, `GET /inbox/messages/{id}`, `POST /inbox/messages/{id}/place`, `DELETE /inbox/messages/{id}`, plus the six `ErrorCode` members. Hand placement re-points the documents directly and creates no action item, so `inbound_action_item` is not needed in this phase. Verify: API tests for owner-only access, CSRF on the unsafe methods, `404` for another owner's message, placement re-pointing the attachment row with **no byte copy** (assert the blob row's identity is unchanged) and re-checking the per-parent and trip limits, and delete cascading to documents.
...
12. The quarantine's 30-day purge, run on the poll. Verify: tests that rows older than the window are deleted, that nothing else is, and that no stranger's body or attachment was ever stored to purge.
13. The `/inbox` screen — the three regions, the empty state naming the address, hand placement, and the `AppShell` badge as one optional prop. Verify: component tests for each region, the unconfigured state, both locales, the ICU plural's Polish `few`/`many` forms, and `check_css_tokens.py` + `check_contrast.py` green.
...
2. `integrations/model.py` — one provider, one prompt, a strict JSON schema, timeout, bounded retry, a budget cap, and the defined failure mode. Verify: tests with a stubbed provider for a good response, malformed JSON, an unknown field, a timeout, a 429, and budget exhaustion — each asserting the message's resulting state and that no partial proposal is built. **No real network call in the suite.**
3. Wiring routing into the poll: `received` → `routed` / `unrouted`, `attempts`, `routing_reason` as **translated keys with arguments**, and `text_sent_at` stamped whenever anything is sent. Verify: tests that a trip id outside the candidate set is rejected; that the cap moves a message to the queue with a reason; that `text_sent_at` is set exactly when egress happened; that the request body carries **only** the message text, its three headers and the candidate trips' id/title/destinations/dates — asserted against a captured payload; and that **no attachment byte is sent in this phase**, asserted the same way.
...
4. Action items created from routing, for both kinds. Verify: tests that a matched item yields `update_item` with alternatives recorded, that no match yields `create_item` on the right day, and that a message carrying only a document yields an `update_item` claiming zero fields and one document — still approved, because putting a document on an item is a write.
...
1. `POST /inbox/messages/{id}/read-document` — the explicit egress action: one named attachment, that attachment's `sent_externally_at` stamped, a re-proposal at `revision + 1`. Verify: tests that no document is ever sent without this call; that the stamp lands on **that** attachment and no other; that the sheet shows which documents have left; and that a sheet opened before the re-proposal is refused on its stale `expected_revision`.
2. The quarantine surface — the count, *Show* revealing sender and subject, and *Trust this sender* adding the address and re-fetching the message by its UID. (The purge itself shipped in Phase 1.) Verify: tests that **no stranger's body or attachment is returned by any route**, that *Show* returns only the three headers, and that releasing a sender re-fetches and processes the message normally.
