...
- Date: 2026-09-05 (realigned 2026-09-06 against the merged design system) · Author: `om-auto-write-spec` (autonomous) · Status: **ready — A2 and A6 confirmed by Michal Klosinski on 2026-09-06; no assumption is gated**
...
This is also the first feature in the product that accepts bytes from a browser, on an installation that D14 puts on the public internet from day one. The Security section is therefore not a checklist at the end; it is a first-class part of the design. The one thing it could not make safe — **inline PDF preview** — is cut and named rather than promised, and the two residual risks it accepts — no malware scanning and no EXIF stripping — are stated plainly, with the condition that would change each.
...
| Apple Wallet / Google Pass export, PDF and Calendar export | **D12** (*Later* list) | "Zsynchronizuj z Apple Wallet & Google Pass", "Eksportuj portfel" — **not designed here.** (The separate question of whether PKPASS is an *accepted upload format* is answered by A3 under the open Q03, not by this row: a *Later* row is not authority for narrowing a *Now* capability) | 
...
- **Postgres large objects (`lo`) instead of `BYTEA`.** They stream, which `BYTEA` does not, and at a 10 MB cap that buys nothing but a second access API and `lo_unlink` bookkeeping that is *not* covered by `ON DELETE CASCADE` — reintroducing exactly the orphan problem this design just avoided.
- **A separate `reservation` table, one-to-one with `item`.** Rejected: three nullable fields with no cardinality above one, bought with a join on every timeline read and a second entity in the glossary. When cost splitting arrives (D12) it will want a *participant* dimension, and that is a new table with its own shape either way — nothing is saved by guessing at it now.
...
- **`get_owned_trip` remains the only fence, and nothing routes around it.** Every attachment route is nested under `/trips/{tripId}/…` and takes the settled dependency, unchanged in name and signature, so the foundation spec's Phase 1 route-enumeration test — *"every `/trips/…` route carries `get_owned_trip`"* — keeps passing unmodified and keeps being the proof of R08. This is also *the reason pre-signed URLs were rejected*: the moment a URL grants access without passing that dependency, that test stops being able to prove anything.
- **The future guest seam lives *inside* `get_owned_trip`, not beside it.** When the sharing spec adds guest access it will widen that one function's body — resolving an owner session *or* a trip grant — rather than introducing a second dependency the enumeration test does not know about. Naming a parallel `resolve_trip_access` dependency would create exactly the second, weaker path this spec spent the object-store argument avoiding. **This spec adds nothing for the magic link**; it only records where the change belongs so that the next spec does not thread a second auth path through the handlers.
...
| Column | Type | Notes |
|---|---|---|
| `confirmation_number` | TEXT NULL, `CHECK (confirmation_number <> '' AND length(confirmation_number) <= 500)` | free text. Vendors use `SX-9912L`, `#9842103`, `TP1205/PNR` and worse, so any *format* we imposed would be wrong for something the user is copying off a voucher — but a length bound is not a format, and this is the only otherwise-unbounded input in the feature |
...
**Two Alembic revisions**, each in the same PR as the code that needs it, each with a working `downgrade`. The first (Phase 1) creates `attachment`, `attachment_blob` and `upload_event`. The second (Phase 3) adds the three nullable columns and their check constraints to `item`. They are split rather than merged so that reservation data is rollback-able without taking attachments with it, and so that the `item` columns never exist during a phase that cannot populate them — see A1. Against `BACKWARD_COMPATIBILITY.md` §2: the new tables did not exist, so "safe against rows that already exist" is vacuous for them; the `item` columns are **nullable with no default and no backfill**, which is the safe form of the rule. The `downgrade` drops the tables and the columns, and therefore destroys the data in them — which is what a downgrade of a feature migration means and is stated here so nobody discovers it at 02:00. It is not the expand/contract case §2 governs: nothing is being removed from a surface a consumer already depends on.
...
| `invalid_reservation_field` | `422` | a confirmation number longer than 500 characters |
| `days_have_attachments` | `409` | a trip's date range would be shortened past a day carrying attachments (see Edge Cases) |
...
D14 puts this on the public internet and this is the first feature that accepts bytes from a browser, so this section is part of the design, not an appendix to it. **Where D15 stops applying, stated once.** D15 (one user) is this document's authority for shipping no malware scanning, no EXIF stripping and no optimistic locking, and it would be arbitrary to invoke it there and then build a rate limiter, an `upload_event` table and an installation-wide quota for that same single user. The line is not the number of users, it is who the control protects *from whom*: a control that protects the owner from **himself** is machinery without a failure to prevent and is cut (D15); a control that protects the deployment from **someone who is not the owner** — a stolen session, a cross-site request, a scanner on the public internet — is required by D14 and is kept. Scanning and EXIF sit on the first side of that line only for as long as nobody but the owner can fetch a byte, which is exactly what A8 turns on.
...
6. **a separate ASCII fallback is derived for the `Content-Disposition` header** by replacing every character outside `[A-Za-z0-9._-]` with `_`. This step is not cosmetic: the header's `filename="…"` parameter is a quoted string, and a name like `a";x=".pdf` would close the quote and inject a parameter. Stripping control characters — step 3 — does not close that hole, because `"` and `\` are not control characters. The UTF-8 form goes in `filename*` where it is percent-encoded; the raw string never reaches a header.
...
|---|---|---|
| PDF | `%PDF-` at offset 0, version `1.0`–`2.0` | an `%%EOF` marker within the last 1024 bytes; total length ≥ 100 bytes. **The PDF is not parsed** — no object graph is walked, so no PDF parser CVE is reachable. The `%%EOF` check is an *integrity* heuristic that catches truncated uploads; it resists no attacker and must not be leaned on as though it did |
...
**No image library runs on the server.** Pillow was the obvious alternative and it lost: `Image.open()` plus `verify()` is a decoder touching attacker-controlled bytes, historically the richest CVE surface in this whole area, and it exists to give us something we do not need. Reading two 32-bit integers out of a PNG `IHDR` is not a decode. The declared pixel count is bounded at **25 megapixels** — a decompression bomb is a small file that expands catastrophically *when decoded*, and since the server never decodes, the bound exists to protect the **browser** that will. 25 MPx is roughly 100 MB of decoded raster, and it is comfortably above any phone camera; the earlier draft's 50 MPx was an order of magnitude above where the protection actually bites, which is a bound that exists on paper only. A file whose header claims 60 000 × 60 000 is refused as `unsupported_file_type`.
...
**The rate window is checked *before the body is read*, not only before the insert.** This matters and the first draft of this spec got it wrong: a limiter that runs after 10 MB has already been pulled into memory refuses to *store* the flood but not to *absorb* it, so a stolen session — the named adversary — could force unbounded repeated 10 MB reads while every quota politely declined. The order is therefore: authenticate → check the rate window → read and count the body → sniff → structurally check → open the transaction → re-check the window and the byte quotas → insert. The pre-read check is the memory control; the in-transaction re-check is the correctness one.

**The byte quotas need an explicit lock, because being in one transaction is not enough.** Under PostgreSQL's default `READ COMMITTED`, two concurrent uploads each `SUM(byte_size)` over committed rows, neither sees the other's uncommitted insert, and both pass a quota only one of them fits in — atomicity is not mutual exclusion. The upload transaction therefore takes `pg_advisory_xact_lock(hashtext(trip_id))` before summing, which serialises uploads *within one trip* and nothing else, and is released by the commit. The installation-wide 2 GB cap takes a second, fixed advisory key. `SERIALIZABLE` with a retry loop was the alternative and it loses: it makes every unrelated request in the application a potential serialisation failure to handle, in exchange for a guarantee two advisory locks already give.
...
| Response-header injection via a filename | two mechanisms, because one is not enough: `filename*` is RFC 5987 percent-encoded, and the `filename=` quoted-string fallback is reduced to `[A-Za-z0-9._-]` (step 6), so neither `"` nor `\` nor a control character can reach a header |
...
| Decompression / pixel bomb | no archive format is accepted; declared pixel count bounded at 25 MPx before the browser ever sees it |
...
**It is never auto-expanded, and nothing ever opens it for the user.** An earlier draft of this spec had the panel spring open the first time an attachment landed on an item, on the reasoning that this is the one second in the product when the user is demonstrably looking at a voucher. That was a prompt to complete reservation details, fired by the app, on a surface the user did not open — which is invariant 4 above, broken by the very design that states it. It was also worse than it looked: a dismissal remembered "per item" means a thirty-item trip prompts thirty times, and nothing in the data model or the API persisted that dismissal, so it would have returned after every reload.

What replaces it is discoverability rather than a nudge: the disclosure sits directly beneath the attachment list, permanently, with its label visible and the word *optional* in it. A user who has just attached a voucher is already looking at that part of the screen. If that turns out to be too quiet in real use on the Malaysia trip, the fix is a one-line change to a label — and it is the owner's call to make after using it, not this spec's to guess before.
...
| A day or item id in the path that is not in this trip | a plain `404`, the same one the foundation spec already returns for a day outside a trip. **Deliberately not a new error code** — one condition, one code |
...
| A confirmation number longer than 500 characters | `422 invalid_reservation_field`. The format is free text because vendors have no common one, but it is the only user input in this document that would otherwise be unbounded, and 500 characters is far beyond any real voucher code. Below the bound it is stored verbatim and rendered truncated with the full value available |
...
| A guest with a magic link requests attachment content | not reachable: no guest route exists in this spec. When the sharing spec adds one, it widens the settled `get_owned_trip` dependency rather than adding a second one, under the recommendations in A8 and A9 |
...
Nothing here *contradicts* an active entry, and nothing needs superseding. But three of the answers below close questions whose text currently lives **inside** active rows — R04 says "whether the first version handles more than one currency is undecided", R05 says "whether the guest sees attachments and confirmation numbers are still undecided (Q03)" — and a spec that answers a question without closing the row leaves the rule text stale, which is how a later reader ends up believing two things at once. The brief's own mechanism for this is a decision record, and the owner is the decider. **Approving this spec's ⚠ assumptions is therefore also a request to file three rows** (numbering follows D15):

| Proposed | Records | Which row it updates |
|---|---|---|
| **D16** | Attachments are stored in the application's own PostgreSQL database; accepted formats are PDF, JPEG and PNG; the per-file limit is 10 MB (A2, A3, A4) | closes the storage/format/size half of brief **Q03** |
| **D17** | Every stored amount carries an ISO-4217 currency code; the first version performs no conversion and no totals (A7) | closes brief **Q04** and the "undecided" sentence in **R04** |
| **D18** | A trip's magic link does not expose attachments in the first version; guest visibility is revisited only together with link revocation, EXIF handling and a scanning decision (A8, A9) | closes the attachment half of brief **Q03** and updates **R05** |

Filing them is one edit to the brief and three short records; it is not a precondition for merging this spec, and this spec does not presume the answers. **In particular, A9's recommendation about revocation is a recommendation** — a specification cannot bind another specification's scope, only a decision row can, which is why D18 exists as a request rather than as a rule stated here.

The approval this document needs is on its autonomous assumptions below.

## ⚠️ Resolved assumptions (autonomous defaults)

This spec was written in `--autonomous` mode. Each question below was resolved by the most reversible, smallest-scope answer available. **A2 and A6 were the two gated on human confirmation, and both were confirmed by Michal Klosinski on 2026-09-06** when he directed this spec to `main`; the remainder stand as documented defaults, revisable by a later spec or decision row.

| # | Question | Resolved as | Rationale |
|---|---|---|---|
| A1 | One spec, or one for attachments and one for reservation data? | **One spec, four phases — with Phase 3 on its own Alembic revision and its own merge gate** | They are separable, and an honest reading has to concede more than the first draft did: they share no table, no endpoint (reservation data rides the *existing* item `PATCH`) and no security control, and the reservation panel works with no attachment at all. What binds them is R04 at the requirement level — the data is defined as arriving *with* material — plus one screen and one review in which the two halves can be judged against each other. **The two concessions that keep the bundle honest:** Phase 3 ships its own migration rather than riding Phase 1's, so it is genuinely rollback-able alone; and the two ⚠ gates below are **independently overridable** — a reviewer who rejects A2 (where the bytes live) blocks the attachment half and not reservation data, which has nothing to do with where bytes live |
| A2 | Where do the bytes live? (brief **Q03**) | **PostgreSQL `BYTEA`, in an `attachment_blob` table separate from the metadata** — ✅ **confirmed by Michal Klosinski, 2026-09-06** | One durable store, one backup, one credential, one failure mode; deleting a trip deletes its files transactionally with no orphan reaper; and no bucket to provision before 2026-09-30 (A05). The move to an object store is additive when it is justified — a `storage_backend` column, an `object_key` column, a copy job. Was marked for confirmation because the brief leaves Q03 explicitly open, because it is the one decision here that is a data migration rather than an addition, and because it is the choice a reviewer is most likely to disagree with. Confirmed |
...
| A6 | Where does reservation data hang, and what exactly is stored? | **Three nullable columns on `item`** — `confirmation_number`, `cost_amount`, `cost_currency` — **and the reservation's *dates* are the item's existing time span, not new columns** — ✅ **confirmed by Michal Klosinski, 2026-09-06** | On the item, because hanging it on the attachment means deleting a voucher silently deletes the confirmation number, and two documents for one booking give one stay two costs. Dates on the item's span, because the foundation spec already refused to model the same journey twice and this is the same refusal. Was marked for confirmation because it is migration-class, because it is the shape any later cost-splitting feature (D12) builds on, and because only the owner knows whether he thinks of a reservation as a property of the plan item or of the document. Confirmed |
| A7 | Does the first version handle more than one currency? (brief **Q04**) | **Every stored amount carries an ISO-4217 code; there is no conversion, no total, no sum and no PLN/EUR toggle anywhere.** The input always defaults to `PLN` | This is the *smaller* thing, not the safer-bigger one. What makes multi-currency expensive is arithmetic — rates, a base currency, cross-currency sums — and none of that is in scope, because totals are D12's. What is left is a three-character column that costs nothing and an `Intl.NumberFormat` call. The alternative, a bare number whose unit lives in someone's head, is `BACKWARD_COMPATIBILITY.md`'s named worst case pre-installed. The export's toggle is dropped: it implies conversion. A "last currency used on this trip" default was considered and dropped: it needs a derived payload field and a query to support a convenience worth one keystroke. **Override note:** if the answer is "PLN only, forever", the change is to narrow the column's `CHECK` — a one-line migration, not a redesign |
| A8 | Does a trip's magic link expose attachments? (brief **Q03**) | **No — not in the first version.** No unauthenticated read path exists in this spec. No unauthenticated read path is added, and the place guest access would later be granted is the body of the settled `get_owned_trip` dependency — not a new one | An unauthenticated bearer URL to a ticket PDF carrying a confirmation number is the highest-consequence thing this feature could leak, and this spec ships **no malware scanning** and **no EXIF stripping** — both tolerable for an owner reading his own files, neither tolerable for a link pasted into a group chat. **A spec cannot bind another spec's scope — only a decision row can**, which is why this is recorded below as proposed decision row **D18** for the owner rather than as a rule this document imposes. The recommendation it carries: guest-visible attachments should require revocable links, a decision on EXIF, and a decision on scanning, first |
...
1. An Alembic revision creating `attachment` (with the exactly-one-parent `CHECK`, both cascading FKs, the `content_type` and `byte_size` checks and the `sha256` index), `attachment_blob`, and `upload_event`. **The `item` reservation columns are deliberately not in this revision** — they belong to Phase 3 (A1), so that phase can be rolled back on its own and so Phases 1–2 do not carry nullable columns nothing can populate, which is the "inventory nobody updates" the foundation spec refused. Verify: an upgrade/downgrade round-trip test; a test that the database itself rejects an attachment with two parents, one with none, and a fourth content type.
2. `domain/uploads.py` — `sniff_type(head: bytes)`, the per-format structural checks, `png_dimensions` / `jpeg_dimensions` read from the header with stdlib code only, the 25 MPx bound, and `normalise_filename`. Verify: unit tests over a fixture corpus — a real minimal PDF, JPEG and PNG; a JPEG named `.pdf`; an HTML page named `.jpg`; an SVG; a PKPASS/ZIP; a truncated PDF with no `%%EOF`; a PNG whose `IHDR` claims 60 000 × 60 000; a zero-byte file; and filenames containing `../`, a newline, 4 000 emoji, and only dots. **No image library is added to `pyproject.toml`** — a test asserts that.
...
4. `security/quota.py` — the per-owner rate and volume windows over `upload_event` (callable **before** the body is read as well as inside the transaction), and the per-parent, per-trip and per-installation byte and count checks taken under `pg_advisory_xact_lock`. Verify: tests that each limit engages at its boundary; that old rows are pruned; that the rate window refuses a request **without reading its body**; and that two genuinely concurrent transactions cannot both consume the last of a trip's quota — a test that fails against an unlocked `SUM` and is the reason the lock is specified.
...
6. `POST /trips/{tripId}/days/{date}/attachments` and `POST /trips/{tripId}/items/{itemId}/attachments`, in the order fixed under Limits: rate window **before** the read, `Content-Length` rejection before the read, byte counting during it, one part only, sniffing, structural check, then the transaction — advisory lock, re-checked window and byte quotas, blob insert. Verify: API tests for a successful upload of each accepted type; for each refusal in step 2's corpus; for a `Content-Length` that lies (chunked, no length, oversized body); for two file parts; for a missing CSRF token; for another owner's trip answering `404`; and that the stored `content_type` is the derived one when the client sent a false header.
...
1. Its own Alembic revision adding `confirmation_number`, `cost_amount`, `cost_currency` to `item`, with the length, non-negative, ISO-4217 and paired-nullability `CHECK`s. Verify: an upgrade/downgrade round-trip against a database already holding items and attachments; tests that the database itself rejects a cost amount without a currency, a negative amount, and a 501-character confirmation number.
2. The `ReservationPanel` disclosure inside the item editor — confirmation number, amount, currency; collapsed by default; nothing required. Verify: component tests that an item saves with every field empty, that the panel is collapsed on first render, and that no validation error can originate from an empty reservation field.
3. The disclosure's placement directly beneath the item's attachment list, with its label and the word *optional* visible while collapsed. Verify: component tests that it is collapsed on every render including immediately after an upload, that **nothing** in the application ever opens it programmatically, and that it does not exist at all for a day-level attachment list.
4. The currency input defaulting to `PLN`, and `Intl.NumberFormat` currency rendering in both locales. Verify: tests asserting `1 250,00 zł` under `pl` and `PLN 1,250.00` under `en` from one call.
5. The status control's path to *gotowe* left untouched and unconditioned. Verify: a test that moving an item to `done` issues no reservation-related request, shows no prompt, and changes the counter — and a test that the counter is identical for two `done` items, one with reservation data and one without. **This is R04's "never demanded" expressed as a test.**
6. Assert the absence of nagging. Verify: tests that no "incomplete", "missing" or "complete your details" string exists in either locale file, and that neither the timeline nor the day detail renders any marker keyed on an empty reservation field.
7. End-to-end verification of the brief's own flow: open a day → open an item → set its details → attach a voucher PDF → save the confirmation number and cost → move the status to *gotowe* → the readiness counter changes. Verify: an integration test walking that path, with screenshots on the implementation PR.
