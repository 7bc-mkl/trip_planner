...
| `mockup-03-states.*` | Four states: **A** hand placement from the unrouted queue, **B** the normal empty state naming the address, **C** the inbox not configured (an explanation, not an error), **D** a failed poll with the honest "messages are not lost" line. Polish |
